...
namespace OmnifariusMod
{
    public class OmnifariusWorld : ModWorld
    {
        ...
        public override void ModifyWorldGenTasks(List<GenPass> tasks, ref float totalWeight)
        {
            ...
            {
                progress.Message = "Creating Volcanoes\n\n\n[Gabe's Test Mod]";
                progress.Set(0.7f);
                int rand1 = Main.spawnTileX + (Main.rand.Next(350, 600));
                ...
                for (int i = 0; i < Main.rand.Next(70, 76); i++)
                {
                    if (Main.tile[rand1, rand2].type != -1)
                    {
                        ...
                        WorldGen.TileRunner(rand1, rand2, 1.1, Main.rand.Next(1, 3), mod.TileType("PetrifiedStoneTile"), false, 0f, 0f, true, true);
                    }
                }
                for (int i = 0; i < 3; i++)
                {
                    MakeVolcano(rand1 + Main.rand.Next(0, 100), rand2);
                }
                
                for (int i = rand1 - 100; i < rand1 + 500; i++)
                {
                    for (int k = rand2; k < rand2 + 400; k++)
                    {
        ...
        {// WorldGen.TileRunner(Main.spawnTileX + 376, Main.spawnTileY + -15, 6, Main.rand.Next(1, 3), mod.TileType("PetrifiedStoneTile"), true, 0f, 0f, true, true);
            ...
            WorldGen.digTunnel(X, Y, 0, 0, 5, 20, false);
        }
    }
}
